...
class QAChain:
    """URL 할루시네이션 방지 QA Chain"""
    
    def __init__(self, api_key: str, retriever, prompt):
        """
        Args:
            api_key: OpenAI API 키
            retriever: HybridRetriever 객체
            prompt: 프롬프트 템플릿
        """
        self.llm = ChatOpenAI(
            model="gpt-4o-mini",
    ...
    def invoke(self, inputs):
        ...
        # 1. 문서 검색
        docs, topic, weights = self.retriever.get_filtered_results(query)
...
def create_hybrid_qa_chain(api_key: str, retriever, prompt, query: str = None):
    """
    URL 할루시네이션 완전 방지 QA Chain 생성
    ...
    if query:
        hybrid_ret, topic, weights, search_filter = retriever.get_hybrid_retriever(query)
        print(f"Retriever: {topic} (BM25={weights[0]}, Vector={weights[1]})")
    
    qa_chain = QAChain(api_key, retriever, prompt)
